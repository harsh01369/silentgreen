 ...
 */
export type DegeneratePattern = 'empty-string' | 'unrendered-template' | 'model-refusal' | 'error-text-in-value' | 'null-literal' | 'truncation-marker';
...
export type AssertionParams = {
    ...
    readonly kind: 'referential';
    readonly sourceField: string;
    readonly sinkField: string;
} | {
    readonly kind: 'not-degenerate';
    readonly fields: readonly string[];
    readonly patterns: readonly DegeneratePattern[];
} | {
    readonly kind: 'cadence';
    readonly expectEverySeconds: number;
...
export type UnprovenReason = 'no-assertions' | 'only-proposed-assertions' | 'contract-stale' | 'sink-not-captured' | 'run-data-unavailable' | 'assertion-not-applicable';
