...
function evidenceHash(input, summary) {
  const material = JSON.stringify({
    ...
    tasks: input.records.map((r) => ({
      id: r.id,
      out: createHash3("sha256").update(r.output).digest("hex"),
      src: createHash3("sha256").update(groundingSourcesFor(r).sources.join("\n")).digest("hex")
    })),
...
function renderEvidenceReport(input) {
  ...
  const rows = (r) => r.problems.map((p) => {
    ...
    return `<div><span class="mono">${esc(p.kind)}</span>${at}<br>${esc(summary2)}<br><span class="meta">${esc(ev)}</span></div>`;
...
${violated.length === 0 ? '<p class="meta">No answer in this batch carried a problem these checks can detect. This is not a statement that the answers are correct.</p>' : violated.map((r) => `<div class="finding"><span class="id">${esc(r.id)}</span>${rows(r)}</div>`).join("")}
...
${broken.length === 0 ? "" : broken.slice(0, 50).map(
    (t) => `<div class="finding"><span class="id">${esc(t.id)}</span>${t.clauses.filter((c) => c.verdict === "violated").map((c) => `<div>${esc(c.clause)}: ${esc(detail(c.detail))}${c.evidence && !redact ? `<br><span class="meta">${esc(c.evidence)}</span>` : ""}</div>`).join("")}</div>`
  ).join("")}`;
}
