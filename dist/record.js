...
function nearestSourceNumber(value, sourceNumbers) {
  ...
  for (const key of sourceNumbers) {
    ...
    const sameDigitsReordered = String(Math.round(Math.abs(n))).length === digits.length && String(Math.round(Math.abs(n))).split("").sort().join("") === digits.split("").sort().join("") && ratio >= 0.5 && ratio <= 2;
