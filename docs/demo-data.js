window.SG = {
  "workflowName": "Shopify orders to Postgres and confirmation email",
  "weeks": 6,
  "platform": {
    "executions": 207,
    "succeeded": 207,
    "failed": 0,
    "sentence": "207 executions, 207 successful, 0 failed."
  },
  "cells": [
    {
      "at": "2026-07-06T09:00:00.000Z",
      "platform": "success",
      "verdict": "proven"
    },
    {
      "at": "2026-07-06T10:00:00.000Z",
      "platform": "success",
      "verdict": "proven"
    },
    {
      "at": "2026-07-06T11:00:00.000Z",
      "platform": "success",
      "verdict": "proven"
    },
    {
      "at": "2026-07-06T12:00:00.000Z",
      "platform": "success",
      "verdict": "proven"
    },
    {
      "at": "2026-07-06T13:00:00.000Z",
      "platform": "success",
      "verdict": "proven"
    },
    {
      "at": "2026-07-06T14:00:00.000Z",
      "platform": "success",
      "verdict": "proven"
    },
    {
      "at": "2026-07-06T15:00:00.000Z",
      "platform": "success",
      "verdict": "proven"
    },
    {
      "at": "2026-07-06T16:00:00.000Z",
      "platform": "success",
      "verdict": "proven"
    },
    {
      "at": "2026-07-06T17:00:00.000Z",
      "platform": "success",
      "verdict": "proven"
    },
    {
      "at": "2026-07-07T09:00:00.000Z",
      "platform": "success",
      "verdict": "proven"
    },
    {
      "at": "2026-07-07T10:00:00.000Z",
      "platform": "success",
      "verdict": "proven"
    },
    {
      "at": "2026-07-07T11:00:00.000Z",
      "platform": "success",
      "verdict": "proven"
    },
    {
      "at": "2026-07-07T12:00:00.000Z",
      "platform": "success",
      "verdict": "proven"
    },
    {
      "at": "2026-07-07T13:00:00.000Z",
      "platform": "success",
      "verdict": "proven"
    },
    {
      "at": "2026-07-07T14:00:00.000Z",
      "platform": "success",
      "verdict": "proven"
    },
    {
      "at": "2026-07-07T15:00:00.000Z",
      "platform": "success",
      "verdict": "proven"
    },
    {
      "at": "2026-07-07T16:00:00.000Z",
      "platform": "success",
      "verdict": "proven"
    },
    {
      "at": "2026-07-07T17:00:00.000Z",
      "platform": "success",
      "verdict": "proven"
    },
    {
      "at": "2026-07-08T09:00:00.000Z",
      "platform": "success",
      "verdict": "proven"
    },
    {
      "at": "2026-07-08T10:00:00.000Z",
      "platform": "success",
      "verdict": "proven"
    },
    {
      "at": "2026-07-08T11:00:00.000Z",
      "platform": "success",
      "verdict": "proven"
    },
    {
      "at": "2026-07-08T12:00:00.000Z",
      "platform": "success",
      "verdict": "proven"
    },
    {
      "at": "2026-07-08T13:00:00.000Z",
      "platform": "success",
      "verdict": "proven"
    },
    {
      "at": "2026-07-08T14:00:00.000Z",
      "platform": "success",
      "verdict": "proven"
    },
    {
      "at": "2026-07-08T15:00:00.000Z",
      "platform": "success",
      "verdict": "proven"
    },
    {
      "at": "2026-07-08T16:00:00.000Z",
      "platform": "success",
      "verdict": "proven"
    },
    {
      "at": "2026-07-08T17:00:00.000Z",
      "platform": "success",
      "verdict": "proven"
    },
    {
      "at": "2026-07-09T09:00:00.000Z",
      "platform": "success",
      "verdict": "proven"
    },
    {
      "at": "2026-07-09T10:00:00.000Z",
      "platform": "success",
      "verdict": "proven"
    },
    {
      "at": "2026-07-09T11:00:00.000Z",
      "platform": "success",
      "verdict": "proven"
    },
    {
      "at": "2026-07-09T12:00:00.000Z",
      "platform": "success",
      "verdict": "proven"
    },
    {
      "at": "2026-07-09T13:00:00.000Z",
      "platform": "success",
      "verdict": "proven"
    },
    {
      "at": "2026-07-09T14:00:00.000Z",
      "platform": "success",
      "verdict": "proven"
    },
    {
      "at": "2026-07-09T15:00:00.000Z",
      "platform": "success",
      "verdict": "proven"
    },
    {
      "at": "2026-07-09T16:00:00.000Z",
      "platform": "success",
      "verdict": "proven"
    },
    {
      "at": "2026-07-09T17:00:00.000Z",
      "platform": "success",
      "verdict": "proven"
    },
    {
      "at": "2026-07-10T09:00:00.000Z",
      "platform": "success",
      "verdict": "proven"
    },
    {
      "at": "2026-07-10T10:00:00.000Z",
      "platform": "success",
      "verdict": "proven"
    },
    {
      "at": "2026-07-10T11:00:00.000Z",
      "platform": "success",
      "verdict": "proven"
    },
    {
      "at": "2026-07-10T12:00:00.000Z",
      "platform": "success",
      "verdict": "proven"
    },
    {
      "at": "2026-07-10T13:00:00.000Z",
      "platform": "success",
      "verdict": "proven"
    },
    {
      "at": "2026-07-10T14:00:00.000Z",
      "platform": "success",
      "verdict": "proven"
    },
    {
      "at": "2026-07-10T15:00:00.000Z",
      "platform": "success",
      "verdict": "proven"
    },
    {
      "at": "2026-07-10T16:00:00.000Z",
      "platform": "success",
      "verdict": "proven"
    },
    {
      "at": "2026-07-10T17:00:00.000Z",
      "platform": "success",
      "verdict": "proven"
    },
    {
      "at": "2026-07-13T09:00:00.000Z",
      "platform": "success",
      "verdict": "proven"
    },
    {
      "at": "2026-07-13T10:00:00.000Z",
      "platform": "success",
      "verdict": "proven"
    },
    {
      "at": "2026-07-13T11:00:00.000Z",
      "platform": "success",
      "verdict": "proven"
    },
    {
      "at": "2026-07-13T12:00:00.000Z",
      "platform": "success",
      "verdict": "proven"
    },
    {
      "at": "2026-07-13T13:00:00.000Z",
      "platform": "success",
      "verdict": "proven"
    },
    {
      "at": "2026-07-13T14:00:00.000Z",
      "platform": "success",
      "verdict": "proven"
    },
    {
      "at": "2026-07-13T15:00:00.000Z",
      "platform": "success",
      "verdict": "proven"
    },
    {
      "at": "2026-07-13T16:00:00.000Z",
      "platform": "success",
      "verdict": "proven"
    },
    {
      "at": "2026-07-13T17:00:00.000Z",
      "platform": "success",
      "verdict": "proven"
    },
    {
      "at": "2026-07-14T09:00:00.000Z",
      "platform": "success",
      "verdict": "proven"
    },
    {
      "at": "2026-07-14T10:00:00.000Z",
      "platform": "success",
      "verdict": "proven"
    },
    {
      "at": "2026-07-14T11:00:00.000Z",
      "platform": "success",
      "verdict": "proven"
    },
    {
      "at": "2026-07-14T12:00:00.000Z",
      "platform": "success",
      "verdict": "proven"
    },
    {
      "at": "2026-07-14T13:00:00.000Z",
      "platform": "success",
      "verdict": "proven"
    },
    {
      "at": "2026-07-14T14:00:00.000Z",
      "platform": "success",
      "verdict": "proven"
    },
    {
      "at": "2026-07-14T15:00:00.000Z",
      "platform": "success",
      "verdict": "proven"
    },
    {
      "at": "2026-07-14T16:00:00.000Z",
      "platform": "success",
      "verdict": "proven"
    },
    {
      "at": "2026-07-14T17:00:00.000Z",
      "platform": "success",
      "verdict": "proven"
    },
    {
      "at": "2026-07-15T09:00:00.000Z",
      "platform": "success",
      "verdict": "proven"
    },
    {
      "at": "2026-07-15T10:00:00.000Z",
      "platform": "success",
      "verdict": "proven"
    },
    {
      "at": "2026-07-15T11:00:00.000Z",
      "platform": "success",
      "verdict": "proven"
    },
    {
      "at": "2026-07-15T12:00:00.000Z",
      "platform": "success",
      "verdict": "proven"
    },
    {
      "at": "2026-07-15T13:00:00.000Z",
      "platform": "success",
      "verdict": "proven"
    },
    {
      "at": "2026-07-15T14:00:00.000Z",
      "platform": "success",
      "verdict": "proven"
    },
    {
      "at": "2026-07-15T15:00:00.000Z",
      "platform": "success",
      "verdict": "proven"
    },
    {
      "at": "2026-07-15T16:00:00.000Z",
      "platform": "success",
      "verdict": "proven"
    },
    {
      "at": "2026-07-15T17:00:00.000Z",
      "platform": "success",
      "verdict": "proven"
    },
    {
      "at": "2026-07-16T09:00:00.000Z",
      "platform": "success",
      "verdict": "proven"
    },
    {
      "at": "2026-07-16T10:00:00.000Z",
      "platform": "success",
      "verdict": "proven"
    },
    {
      "at": "2026-07-16T11:00:00.000Z",
      "platform": "success",
      "verdict": "proven"
    },
    {
      "at": "2026-07-16T12:00:00.000Z",
      "platform": "success",
      "verdict": "proven"
    },
    {
      "at": "2026-07-16T13:00:00.000Z",
      "platform": "success",
      "verdict": "proven"
    },
    {
      "at": "2026-07-16T14:00:00.000Z",
      "platform": "success",
      "verdict": "proven"
    },
    {
      "at": "2026-07-16T15:00:00.000Z",
      "platform": "success",
      "verdict": "proven"
    },
    {
      "at": "2026-07-16T16:00:00.000Z",
      "platform": "success",
      "verdict": "proven"
    },
    {
      "at": "2026-07-16T17:00:00.000Z",
      "platform": "success",
      "verdict": "proven"
    },
    {
      "at": "2026-07-17T09:00:00.000Z",
      "platform": "success",
      "verdict": "proven"
    },
    {
      "at": "2026-07-17T10:00:00.000Z",
      "platform": "success",
      "verdict": "proven"
    },
    {
      "at": "2026-07-17T11:00:00.000Z",
      "platform": "success",
      "verdict": "proven"
    },
    {
      "at": "2026-07-17T12:00:00.000Z",
      "platform": "success",
      "verdict": "proven"
    },
    {
      "at": "2026-07-17T13:00:00.000Z",
      "platform": "success",
      "verdict": "proven"
    },
    {
      "at": "2026-07-17T14:00:00.000Z",
      "platform": "success",
      "verdict": "proven"
    },
    {
      "at": "2026-07-17T15:00:00.000Z",
      "platform": "success",
      "verdict": "proven"
    },
    {
      "at": "2026-07-17T16:00:00.000Z",
      "platform": "success",
      "verdict": "proven"
    },
    {
      "at": "2026-07-17T17:00:00.000Z",
      "platform": "success",
      "verdict": "proven"
    },
    {
      "at": "2026-07-20T09:00:00.000Z",
      "platform": "success",
      "verdict": "proven"
    },
    {
      "at": "2026-07-20T10:00:00.000Z",
      "platform": "success",
      "verdict": "proven"
    },
    {
      "at": "2026-07-20T11:00:00.000Z",
      "platform": "success",
      "verdict": "proven"
    },
    {
      "at": "2026-07-20T12:00:00.000Z",
      "platform": "success",
      "verdict": "proven"
    },
    {
      "at": "2026-07-20T13:00:00.000Z",
      "platform": "success",
      "verdict": "proven"
    },
    {
      "at": "2026-07-20T14:00:00.000Z",
      "platform": "success",
      "verdict": "proven"
    },
    {
      "at": "2026-07-20T15:00:00.000Z",
      "platform": "success",
      "verdict": "proven"
    },
    {
      "at": "2026-07-20T16:00:00.000Z",
      "platform": "success",
      "verdict": "proven"
    },
    {
      "at": "2026-07-20T17:00:00.000Z",
      "platform": "success",
      "verdict": "proven"
    },
    {
      "at": "2026-07-21T09:00:00.000Z",
      "platform": "success",
      "verdict": "proven"
    },
    {
      "at": "2026-07-21T10:00:00.000Z",
      "platform": "success",
      "verdict": "proven"
    },
    {
      "at": "2026-07-21T11:00:00.000Z",
      "platform": "success",
      "verdict": "proven"
    },
    {
      "at": "2026-07-21T12:00:00.000Z",
      "platform": "success",
      "verdict": "proven"
    },
    {
      "at": "2026-07-21T13:00:00.000Z",
      "platform": "success",
      "verdict": "proven"
    },
    {
      "at": "2026-07-21T14:00:00.000Z",
      "platform": "success",
      "verdict": "proven"
    },
    {
      "at": "2026-07-21T15:00:00.000Z",
      "platform": "success",
      "verdict": "proven"
    },
    {
      "at": "2026-07-21T16:00:00.000Z",
      "platform": "success",
      "verdict": "proven"
    },
    {
      "at": "2026-07-21T17:00:00.000Z",
      "platform": "success",
      "verdict": "proven"
    },
    {
      "at": "2026-07-22T09:00:00.000Z",
      "platform": "success",
      "verdict": "proven"
    },
    {
      "at": "2026-07-22T10:00:00.000Z",
      "platform": "success",
      "verdict": "proven"
    },
    {
      "at": "2026-07-22T11:00:00.000Z",
      "platform": "success",
      "verdict": "proven"
    },
    {
      "at": "2026-07-22T12:00:00.000Z",
      "platform": "success",
      "verdict": "proven"
    },
    {
      "at": "2026-07-22T13:00:00.000Z",
      "platform": "success",
      "verdict": "proven"
    },
    {
      "at": "2026-07-22T14:00:00.000Z",
      "platform": "success",
      "verdict": "proven"
    },
    {
      "at": "2026-07-22T15:00:00.000Z",
      "platform": "success",
      "verdict": "proven"
    },
    {
      "at": "2026-07-22T16:00:00.000Z",
      "platform": "success",
      "verdict": "proven"
    },
    {
      "at": "2026-07-22T17:00:00.000Z",
      "platform": "success",
      "verdict": "proven"
    },
    {
      "at": "2026-07-23T09:00:00.000Z",
      "platform": "success",
      "verdict": "proven"
    },
    {
      "at": "2026-07-23T10:00:00.000Z",
      "platform": "success",
      "verdict": "proven"
    },
    {
      "at": "2026-07-23T11:00:00.000Z",
      "platform": "success",
      "verdict": "proven"
    },
    {
      "at": "2026-07-23T12:00:00.000Z",
      "platform": "success",
      "verdict": "proven"
    },
    {
      "at": "2026-07-23T13:00:00.000Z",
      "platform": "success",
      "verdict": "proven"
    },
    {
      "at": "2026-07-23T14:00:00.000Z",
      "platform": "success",
      "verdict": "proven"
    },
    {
      "at": "2026-07-23T15:00:00.000Z",
      "platform": "success",
      "verdict": "proven"
    },
    {
      "at": "2026-07-23T16:00:00.000Z",
      "platform": "success",
      "verdict": "proven"
    },
    {
      "at": "2026-07-23T17:00:00.000Z",
      "platform": "success",
      "verdict": "proven"
    },
    {
      "at": "2026-07-24T09:00:00.000Z",
      "platform": "success",
      "verdict": "proven"
    },
    {
      "at": "2026-07-24T10:00:00.000Z",
      "platform": "success",
      "verdict": "proven"
    },
    {
      "at": "2026-07-24T11:00:00.000Z",
      "platform": "success",
      "verdict": "proven"
    },
    {
      "at": "2026-07-24T12:00:00.000Z",
      "platform": "success",
      "verdict": "proven"
    },
    {
      "at": "2026-07-24T13:00:00.000Z",
      "platform": "success",
      "verdict": "proven"
    },
    {
      "at": "2026-07-24T14:00:00.000Z",
      "platform": "success",
      "verdict": "proven"
    },
    {
      "at": "2026-07-24T15:00:00.000Z",
      "platform": "success",
      "verdict": "proven"
    },
    {
      "at": "2026-07-24T16:00:00.000Z",
      "platform": "success",
      "verdict": "proven"
    },
    {
      "at": "2026-07-24T17:00:00.000Z",
      "platform": "success",
      "verdict": "proven"
    },
    {
      "at": "2026-07-27T09:00:00.000Z",
      "platform": "success",
      "verdict": "proven"
    },
    {
      "at": "2026-07-27T10:00:00.000Z",
      "platform": "success",
      "verdict": "proven"
    },
    {
      "at": "2026-07-27T11:00:00.000Z",
      "platform": "success",
      "verdict": "proven"
    },
    {
      "at": "2026-07-27T12:00:00.000Z",
      "platform": "success",
      "verdict": "proven"
    },
    {
      "at": "2026-07-27T13:00:00.000Z",
      "platform": "success",
      "verdict": "proven"
    },
    {
      "at": "2026-07-27T14:00:00.000Z",
      "platform": "success",
      "verdict": "proven"
    },
    {
      "at": "2026-07-27T15:00:00.000Z",
      "platform": "success",
      "verdict": "proven"
    },
    {
      "at": "2026-07-27T16:00:00.000Z",
      "platform": "success",
      "verdict": "proven"
    },
    {
      "at": "2026-07-27T17:00:00.000Z",
      "platform": "success",
      "verdict": "proven"
    },
    {
      "at": "2026-07-28T09:00:00.000Z",
      "platform": "success",
      "verdict": "violated"
    },
    {
      "at": "2026-07-28T10:00:00.000Z",
      "platform": "success",
      "verdict": "violated"
    },
    {
      "at": "2026-07-28T11:00:00.000Z",
      "platform": "success",
      "verdict": "violated"
    },
    {
      "at": "2026-07-28T12:00:00.000Z",
      "platform": "success",
      "verdict": "violated"
    },
    {
      "at": "2026-07-28T13:00:00.000Z",
      "platform": "success",
      "verdict": "violated"
    },
    {
      "at": "2026-07-28T14:00:00.000Z",
      "platform": "success",
      "verdict": "violated"
    },
    {
      "at": "2026-07-28T15:00:00.000Z",
      "platform": "success",
      "verdict": "violated"
    },
    {
      "at": "2026-07-28T16:00:00.000Z",
      "platform": "success",
      "verdict": "violated"
    },
    {
      "at": "2026-07-28T17:00:00.000Z",
      "platform": "success",
      "verdict": "violated"
    },
    {
      "at": "2026-07-29T09:00:00.000Z",
      "platform": "success",
      "verdict": "violated"
    },
    {
      "at": "2026-07-29T10:00:00.000Z",
      "platform": "success",
      "verdict": "violated"
    },
    {
      "at": "2026-07-29T11:00:00.000Z",
      "platform": "success",
      "verdict": "violated"
    },
    {
      "at": "2026-07-29T12:00:00.000Z",
      "platform": "success",
      "verdict": "violated"
    },
    {
      "at": "2026-07-29T13:00:00.000Z",
      "platform": "success",
      "verdict": "violated"
    },
    {
      "at": "2026-07-29T14:00:00.000Z",
      "platform": "success",
      "verdict": "violated"
    },
    {
      "at": "2026-07-29T15:00:00.000Z",
      "platform": "success",
      "verdict": "violated"
    },
    {
      "at": "2026-07-29T16:00:00.000Z",
      "platform": "success",
      "verdict": "violated"
    },
    {
      "at": "2026-07-29T17:00:00.000Z",
      "platform": "success",
      "verdict": "violated"
    },
    {
      "at": "2026-07-30T09:00:00.000Z",
      "platform": "success",
      "verdict": "violated"
    },
    {
      "at": "2026-07-30T10:00:00.000Z",
      "platform": "success",
      "verdict": "violated"
    },
    {
      "at": "2026-07-30T11:00:00.000Z",
      "platform": "success",
      "verdict": "violated"
    },
    {
      "at": "2026-07-30T12:00:00.000Z",
      "platform": "success",
      "verdict": "violated"
    },
    {
      "at": "2026-07-30T13:00:00.000Z",
      "platform": "success",
      "verdict": "violated"
    },
    {
      "at": "2026-07-30T14:00:00.000Z",
      "platform": "success",
      "verdict": "violated"
    },
    {
      "at": "2026-07-30T15:00:00.000Z",
      "platform": "success",
      "verdict": "violated"
    },
    {
      "at": "2026-07-30T16:00:00.000Z",
      "platform": "success",
      "verdict": "violated"
    },
    {
      "at": "2026-07-30T17:00:00.000Z",
      "platform": "success",
      "verdict": "violated"
    },
    {
      "at": "2026-07-31T09:00:00.000Z",
      "platform": "success",
      "verdict": "violated"
    },
    {
      "at": "2026-07-31T10:00:00.000Z",
      "platform": "success",
      "verdict": "violated"
    },
    {
      "at": "2026-07-31T11:00:00.000Z",
      "platform": "success",
      "verdict": "violated"
    },
    {
      "at": "2026-07-31T12:00:00.000Z",
      "platform": "success",
      "verdict": "violated"
    },
    {
      "at": "2026-07-31T13:00:00.000Z",
      "platform": "success",
      "verdict": "violated"
    },
    {
      "at": "2026-07-31T14:00:00.000Z",
      "platform": "success",
      "verdict": "violated"
    },
    {
      "at": "2026-07-31T15:00:00.000Z",
      "platform": "success",
      "verdict": "violated"
    },
    {
      "at": "2026-07-31T16:00:00.000Z",
      "platform": "success",
      "verdict": "violated"
    },
    {
      "at": "2026-07-31T17:00:00.000Z",
      "platform": "success",
      "verdict": "violated"
    },
    {
      "at": "2026-08-03T09:00:00.000Z",
      "platform": "success",
      "verdict": "violated"
    },
    {
      "at": "2026-08-03T10:00:00.000Z",
      "platform": "success",
      "verdict": "violated"
    },
    {
      "at": "2026-08-03T11:00:00.000Z",
      "platform": "success",
      "verdict": "violated"
    },
    {
      "at": "2026-08-03T12:00:00.000Z",
      "platform": "success",
      "verdict": "violated"
    },
    {
      "at": "2026-08-03T13:00:00.000Z",
      "platform": "success",
      "verdict": "violated"
    },
    {
      "at": "2026-08-03T14:00:00.000Z",
      "platform": "success",
      "verdict": "violated"
    },
    {
      "at": "2026-08-03T15:00:00.000Z",
      "platform": "success",
      "verdict": "violated"
    },
    {
      "at": "2026-08-03T16:00:00.000Z",
      "platform": "success",
      "verdict": "violated"
    },
    {
      "at": "2026-08-03T17:00:00.000Z",
      "platform": "success",
      "verdict": "violated"
    },
    {
      "at": "2026-08-04T09:00:00.000Z",
      "platform": "success",
      "verdict": "violated"
    },
    {
      "at": "2026-08-04T10:00:00.000Z",
      "platform": "success",
      "verdict": "violated"
    },
    {
      "at": "2026-08-04T11:00:00.000Z",
      "platform": "success",
      "verdict": "violated"
    },
    {
      "at": "2026-08-04T12:00:00.000Z",
      "platform": "success",
      "verdict": "violated"
    },
    {
      "at": "2026-08-04T13:00:00.000Z",
      "platform": "success",
      "verdict": "violated"
    },
    {
      "at": "2026-08-04T14:00:00.000Z",
      "platform": "success",
      "verdict": "violated"
    },
    {
      "at": "2026-08-04T15:00:00.000Z",
      "platform": "success",
      "verdict": "violated"
    },
    {
      "at": "2026-08-04T16:00:00.000Z",
      "platform": "success",
      "verdict": "violated"
    },
    {
      "at": "2026-08-04T17:00:00.000Z",
      "platform": "success",
      "verdict": "violated"
    },
    {
      "at": "2026-08-05T09:00:00.000Z",
      "platform": "success",
      "verdict": "violated"
    },
    {
      "at": "2026-08-05T10:00:00.000Z",
      "platform": "success",
      "verdict": "violated"
    },
    {
      "at": "2026-08-05T11:00:00.000Z",
      "platform": "success",
      "verdict": "violated"
    },
    {
      "at": "2026-08-05T12:00:00.000Z",
      "platform": "success",
      "verdict": "violated"
    },
    {
      "at": "2026-08-05T13:00:00.000Z",
      "platform": "success",
      "verdict": "violated"
    },
    {
      "at": "2026-08-05T14:00:00.000Z",
      "platform": "success",
      "verdict": "violated"
    },
    {
      "at": "2026-08-05T15:00:00.000Z",
      "platform": "success",
      "verdict": "violated"
    },
    {
      "at": "2026-08-05T16:00:00.000Z",
      "platform": "success",
      "verdict": "violated"
    },
    {
      "at": "2026-08-05T17:00:00.000Z",
      "platform": "success",
      "verdict": "violated"
    }
  ],
  "breakageIndex": 144,
  "counts": {
    "proven": 1737,
    "violated": 127,
    "unproven": 0
  },
  "runsExamined": 207,
  "runsWithViolations": 63,
  "headline": "63 of 207 runs (30%) produced output that violated a confirmed expectation, while the platform recorded 0 failures.",
  "findings": [
    {
      "statement": "Every item from \"Insert into orders\" carries \"order_id\", \"customer_email\", \"total\", \"synced_at\"",
      "basis": "observation",
      "count": 63,
      "firstAt": "2026-07-28T09:00:00.000Z",
      "detail": "Item 0 has field \"customer_email\" but its value is null.",
      "evidence": "{\"order_id\":\"ORD-22-9-0\",\"customer_email\":null,\"total\":161.59,\"synced_at\":\"2026-07-28T09:00:00.000Z\"}"
    },
    {
      "statement": "Text from \"Send confirmation\" is never empty, unrendered or an error string",
      "basis": "observation",
      "count": 63,
      "firstAt": "2026-07-28T09:00:00.000Z",
      "detail": "Item 0, field \"body\" still contains an unrendered template expression, so a value was never substituted in. Also affected in the same run: \"to\" (null-literal).",
      "evidence": "Hi {{ $json.firstName }}, your order is confirmed. Total: {{ $json.total }}"
    },
    {
      "statement": "This workflow runs at least every 60 minutes",
      "basis": "observation",
      "count": 1,
      "detail": "Last run was 11 days ago (2.7 days of working time, once hours it never runs in are discounted). Expected one every 60 min with 60 min of grace. Nothing has failed, because nothing has run.",
      "evidence": "2026-08-05T17:00:00.000Z"
    }
  ],
  "honesty": {
    "live": 10,
    "byBasis": {
      "intent": 0,
      "structure": 3,
      "observation": 7
    },
    "canEstablishCorrectness": false,
    "sentence": "10 live checks, none of which came from a stated business intent. They show the workflow matches its own definition and has not drifted. Whether the definition is right is still unverified."
  },
  "afterEdit": {
    "counts": {
      "proven": 0,
      "violated": 0,
      "unproven": 1864
    },
    "drift": [
      "\"Fetch new orders\" had its parameters changed."
    ]
  },
  "cadenceReasoning": "128 intervals, tightly clustered around 60 min. This looks like a schedule, and a missed run should be obvious quickly. No run has ever started at a weekend, so weekend gaps are excluded from the rhythm rather than treated as incidents. Runs only ever start between 09:00 and 17:59, so overnight silence is expected too. 15 gap(s) that crossed non-working time were excluded from the calculation, which is what keeps the expectation tight enough to be useful.",
  "proposals": [
    {
      "statement": "\"Insert into orders\" produces at least one item on every run",
      "basis": "structure",
      "rationale": "\"Insert into orders\" writes to a database, so an empty run here is data that quietly never arrived. Nothing downstream consumes it, so if it produces nothing the run still ends green.",
      "confidence": "high"
    },
    {
      "statement": "\"Send confirmation\" produces at least one item on every run",
      "basis": "structure",
      "rationale": "\"Send confirmation\" sends email. An empty or half-rendered send reaches a real inbox and cannot be recalled. Nothing downstream consumes it, so if it produces nothing the run still ends green.",
      "confidence": "high"
    },
    {
      "statement": "\"Send confirmation\" never sends an empty, unrendered or refused message",
      "basis": "structure",
      "rationale": "This step delivers text to a person. An unrendered \"{{ $json.firstName }}\" reaching a customer is a visible failure that the platform records as a success.",
      "confidence": "high"
    },
    {
      "statement": "\"Insert into orders\" produces between 20 and 118 items per run",
      "basis": "observation",
      "rationale": "Across 144 successful runs this step produced between 40 and 60 items, typically 50. The bounds are set wide, so this catches a collapse or a runaway rather than ordinary variation.",
      "confidence": "high"
    },
    {
      "statement": "Every item from \"Insert into orders\" carries \"order_id\", \"customer_email\", \"total\", \"synced_at\"",
      "basis": "observation",
      "rationale": "These fields were present and non-null on 100% or more of 7228 captured items. A field that silently stops arriving is one of the commonest causes of downstream rows that look filled in and are not.",
      "confidence": "high"
    },
    {
      "statement": "Text from \"Insert into orders\" is never empty, unrendered or an error string",
      "basis": "observation",
      "rationale": "2 field(s) here carry prose averaging 17 characters. Those are the fields where \"[object Object]\", an unresolved \"{{ }}\" or a model refusal will pass through unnoticed.",
      "confidence": "moderate"
    },
    {
      "statement": "\"Send confirmation\" produces between 20 and 118 items per run",
      "basis": "observation",
      "rationale": "Across 144 successful runs this step produced between 40 and 60 items, typically 50. The bounds are set wide, so this catches a collapse or a runaway rather than ordinary variation.",
      "confidence": "high"
    },
    {
      "statement": "Every item from \"Send confirmation\" carries \"to\", \"subject\", \"body\", \"order_id\"",
      "basis": "observation",
      "rationale": "These fields were present and non-null on 100% or more of 7228 captured items. A field that silently stops arriving is one of the commonest causes of downstream rows that look filled in and are not.",
      "confidence": "high"
    },
    {
      "statement": "Text from \"Send confirmation\" is never empty, unrendered or an error string",
      "basis": "observation",
      "rationale": "3 field(s) here carry prose averaging 17 characters. Those are the fields where \"[object Object]\", an unresolved \"{{ }}\" or a model refusal will pass through unnoticed.",
      "confidence": "moderate"
    },
    {
      "statement": "This workflow runs at least every 60 minutes",
      "basis": "observation",
      "rationale": "128 intervals, tightly clustered around 60 min. This looks like a schedule, and a missed run should be obvious quickly. No run has ever started at a weekend, so weekend gaps are excluded from the rhythm rather than treated as incidents. Runs only ever start between 09:00 and 17:59, so overnight silence is expected too. 15 gap(s) that crossed non-working time were excluded from the calculation, which is what keeps the expectation tight enough to be useful. A workflow that stops firing writes no execution and raises no error, so without this check its disappearance is invisible.",
      "confidence": "high"
    }
  ],
  "sinks": [
    {
      "name": "Insert into orders",
      "category": "datastore",
      "rationale": "\"Insert into orders\" writes to a database, so an empty run here is data that quietly never arrived. Nothing downstream consumes it, so if it produces nothing the run still ends green."
    },
    {
      "name": "Send confirmation",
      "category": "email",
      "rationale": "\"Send confirmation\" sends email. An empty or half-rendered send reaches a real inbox and cannot be recalled. Nothing downstream consumes it, so if it produces nothing the run still ends green."
    }
  ],
  "refusalMessage": "Say how you know that window was good, in a sentence someone could later check. \"Reconciled against the client's invoice export for March\" is an attestation. \"Looks fine\" is not, and it will be printed next to every green tick it produces.",
  "generatedAt": "2026-09-09T12:23:29.795Z"
};
